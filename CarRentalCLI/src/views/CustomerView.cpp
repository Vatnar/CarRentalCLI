...
==================================
    ...
    std::cout << customer->name << "\t\t" << customer->tel << "\t\t" << customer
            ->email << std::endl;
...
{
    std::vector<CustomerModel> customers = customerController.
            SearchCustomer("");
    if (customers.empty())
    {
    ...
    {
        std::cout << index++ << ".\t " << customer.name << "\t\t" << customer.
                tel << "\t\t" << customer.email << std::endl;
    }
}
...
int CustomerView::GetCustomerId()
{
    ...
    std::cout << "Found " << customers.size() << " out of " <<
            customerController.CountCustomers() << std::endl;
    int index = 0;
    for (const auto &customer: customers)
    {
        std::cout << index++ << ".\t " << customer.name << "\t\t" << customer.
    ...
    auto indexCustomer = Input::GetInt(0, customers.size() - 1);
    // -1 cuz of 0 index
